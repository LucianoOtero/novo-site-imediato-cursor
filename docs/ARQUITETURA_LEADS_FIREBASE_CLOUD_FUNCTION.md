...
## Achado paralelo — investigado e corrigido (2026-07-12)

Em todos os testes desta sessão (local e produção), o Octadesk de produção rejeitava consistentemente o payload, mesmo depois de múltiplas rodadas de reentrega da Cloud Function. Investigação isolada (chamada direta ao proxy Cloud Run do Octadesk, fora da aplicação, capturando o corpo completo da resposta de erro) revelou a causa raiz:

- **Bug real em `lib/leads/legacy-proxy-payload.ts`** (existia desde a implementação original, 2026-07-03): o campo `DDD-CELULAR` era montado como `"${ddd}-${celular}"` (ex.: `"11-988887777"`), mas o contrato real dos proxies — confirmado lendo `registrarPrimeiroContatoEspoCRM`/`enviarMensagemInicialOctadesk` em `MODAL_WHATSAPP_DEFINITIVO.js` — espera **só o DDD** nesse campo (ex.: `"11"`), apesar do nome do campo sugerir o contrário.
- **Por que só o Octadesk falhava**: o EspoCRM tolera o formato incorreto (não valida esse campo com o mesmo rigor); o Octadesk valida e rejeita com `HTTP 500 {"success":false,"error":"Erro ao processar webhook","details":"Telefone inválido"}`.
- **Confirmado com chamada isolada real**: `DDD-CELULAR: "11-988887777"` → 500 "Telefone inválido"; `DDD-CELULAR: "11"` → 200 sucesso, sem nenhuma outra mudança no payload.
- **Corrigido** em `lib/leads/legacy-proxy-payload.ts` (usado pelo site) e na cópia duplicada em `firebase/functions/index.js` (Cloud Function). Também melhorado `lib/leads/proxy-sender.ts` para logar o corpo da resposta em qualquer falha futura — a falta desse log foi o que atrasou o diagnóstico deste bug (uma resposta não-OK só gerava um retry silencioso, sem revelar o motivo).
- **Validado de ponta a ponta** após a correção: lead de teste local entregue a EspoCRM e Octadesk **de primeira tentativa** (`octadeskAttempts: 1`, antes sempre falhava após 4 tentativas).
