...
| Habilitar RPA em produção | Você confirmou (2026-07-08) que a TI já aceita CORS de `rpaimediatoseguros.com.br` para o novo domínio — falta só ligar `NEXT_PUBLIC_RPA_ENABLED=true` no ambiente de produção da Vercel, quando chegarmos lá | O `LeadForm` agora tem um passo 4 de decisão ("Aguardar o cálculo" vs. "Falar com consultor depois", projeto 2026-07-16) com a mecânica completa (16 fases, timer, 2 cartões de resultado, fallback de erro) — ver `docs/RPA_ESCOLHA_CALCULO.md`. Só falta ligar a flag para o cálculo automático funcionar de verdade em produção |
