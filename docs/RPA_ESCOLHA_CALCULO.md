...
Implementação: `lib/rpa-calculation.ts` (mensagens/tipo `RpaDisabledReason`), `components/lead/RpaChoiceStep.tsx` (`featureEnabled`/`rpaEnabled`/`rpaDisabledReason`), `components/lead/LeadForm.tsx` (cálculo da elegibilidade + `publicEnv.rpaEnabled`).
